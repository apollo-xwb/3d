...
async function setupViewer(){
    ...
    // Rings catalog elements (All Rings section)
    const ringsOverlay = document.querySelector('.rings-overlay') as HTMLElement | null
    const ringsGrid = document.getElementById('rings-grid') as HTMLElement | null
    const openRingsButton = document.querySelector('.nav--rings') as HTMLElement | null
    const closeRingsButton = document.querySelector('.rings-close') as HTMLButtonElement | null
    const controlsHints = document.querySelector('.controls-hints') as HTMLElement | null

    // Add WEBGi plugins
    await viewer.addPlugin(GBufferPlugin)
    await viewer.addPlugin(new ProgressivePlugin(32))
    // Use default tonemapping without custom color tint to avoid shader issues.
    ...
    // Luxury monochrome background
    viewer.setBackground(new Color('#070708').convertSRGBToLinear())
    ...
    function showControlsHints(){
        if(!controlsHints) return
        gsap.killTweensOf(controlsHints)
        gsap.set(controlsHints, {autoAlpha: 0, y: 10})
        gsap.to(controlsHints, {autoAlpha: 1, y: 0, duration: 0.6, ease: "power3.out"})
        gsap.to(controlsHints, {autoAlpha: 0, y: 10, duration: 0.6, ease: "power3.in", delay: 4})
    }

    function hideControlsHints(){
        if(!controlsHints) return
        gsap.killTweensOf(controlsHints)
        gsap.to(controlsHints, {autoAlpha: 0, y: 10, duration: 0.4, ease: "power3.in"})
    }
    ...
    function setupScrollAnimation(){
        ...
        .fromTo(colorLerpValue, {x:0}, {x:1,
            ...
            , onUpdate: function() {
                if(!usingCustomColors){
                    // Original warm transition for first section
        ...
        .fromTo(colorLerpValue2, {x:0}, {x:1,
            ...
            , onUpdate: function() {
                if(!usingCustomColors){
                    // Original second section transition
    ...
    function toggleNightMode(){
        if(!nightMode){
            ...
            viewer.setBackground(new Color('#f5f5f5').convertSRGBToLinear())
            nightMode = true
        } else{
    ...
    async function loadNewModel(){
        const next = ringModel === 1 ? 2 : 1
        await loadRingByModel(next)
    }

    async function loadRingByModel(targetModel: number){
    ...
    // -----------------------------
    // Rings catalog (All Rings)
    // -----------------------------
    function openRingsCatalog(){
        if(!ringsOverlay) return
        ringsOverlay.classList.add('open')
        ringsOverlay.setAttribute('aria-hidden', 'false')
        document.body.style.overflowY = "hidden"
    }
    function closeRingsCatalog(){
        if(!ringsOverlay) return
        ringsOverlay.classList.remove('open')
        ringsOverlay.setAttribute('aria-hidden', 'true')
        document.body.style.overflowY = "scroll"
    }

    openRingsButton?.addEventListener('click', () => {
        openRingsCatalog()
    })
    closeRingsButton?.addEventListener('click', () => {
        closeRingsCatalog()
    })
    ringsOverlay?.addEventListener('click', (e) => {
        // click outside panel closes
        if(e.target === ringsOverlay) closeRingsCatalog()
    })

    async function ensureRingModel(desired: number){
        if(desired !== 1 && desired !== 2) return
        await loadRingByModel(desired)
    }

    function renderRings(items: RingCatalogItem[]){
        if(!ringsGrid) return
        ringsGrid.innerHTML = ""
        for(const item of items){
            ...
            badge.textContent = item.badge || 'Ring'
            ...
            meta.textContent = item.tagline || (item.href ? item.href : 'Select to load into configurator')
            ...
            card.addEventListener('click', async () => {
                if(item.model){
                    gsap.to('.loader', {x: '0%', duration: 0.6, ease: "power4.inOut", onComplete: async () =>{
                        await ensureRingModel(item.model!)
                        gsap.to('.loader', {x: '100%', duration: 0.6, ease: "power4.inOut", delay: 0.4})
                    }})
                }
                closeRingsCatalog()
                // Jump straight into the configurator, same as "Customize it" CTA.
                const customizeBtn = document.querySelector('.btn-customize') as HTMLElement | null
                customizeBtn?.click()
            })
    ...
    async function loadRingsCatalog(){
        try{
            const res = await fetch('./assets/data/rings.json', { cache: 'no-cache' })
            const json = await res.json()
            if(Array.isArray(json)){
                renderRings(json as RingCatalogItem[])
            }
        }catch{
            renderRings([
